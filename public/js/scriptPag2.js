...
fetch('servizi/check').then(onResponse).then(onJSON);
...
if(hl.dataset.checked==='false'){
    ...
    fetch('aggiungi/servizio/' + cont[id].titolo + '/' + cont[id].image + '/' + id).then(onResponseConf);
...
fetch('rimuovi/servizio/' + titolo.textContent).then(onResponseConf);
  box.remove();
